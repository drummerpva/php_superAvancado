<?php
    class Anuncios extends model{
        ...
        public function getTotalAnuncios($filtros){
            ...
            $sql = $this->db->prepare("SELECT count(1) as total FROM anuncios a WHERE ". implode(" AND ", $filtroString));
        ...
        public function getMeusAnuncios($id){
            $array = array();
            $sql = $this->db->prepare("SELECT a.*, (SELECT ai.url FROM anuncios_imagens ai WHERE ai.id_anuncio = a.id LIMIT 1 ) as url FROM anuncios a WHERE id_usuario = :id");
            $sql->bindValue(":id",$id);
            $sql->execute();
            if($sql->rowCount() > 0){
                $array = $sql->fetchAll();
        ...
        public function getUltimosAnuncios($page,$perPage,$filtros){
            ...
            $sql = $this->db->prepare("SELECT a.*, (SELECT ai.url FROM anuncios_imagens ai WHERE ai.id_anuncio = a.id LIMIT 1 ) as url, "
        ...
        public function alterarAnuncio($idAnuncio,$categoria,$titulo,$valor,$descricao,$estado,$fotos){
            $sql = $this->db->prepare("UPDATE anuncios SET id_categoria=:categoria, titulo=:titulo, valor=:valor, descricao=:descricao, estado=:estado WHERE id = :id");
        ...
        public function inserirAnuncio($idUsuario,$categoria,$titulo,$valor,$descricao,$estado){
            $sql = $this->db->prepare("INSERT INTO anuncios SET id_usuario= :usuario, id_categoria=:categoria, titulo=:titulo, valor=:valor, descricao=:descricao, estado=:estado");
        ...
        public function excluirFoto($id){
            $idAnuncio = 0;
            $sql = $this->db->prepare("SELECT id_anuncio, url FROM anuncios_imagens WHERE id = ?");
            $sql->execute(array($id));
            if($sql->rowCount() > 0){
                $sql = $sql->fetch();
        ...
        public function getAnuncio($id){
            $array = array();
            $sql = $this->db->prepare("SELECT a.*,(SELECT c.nome FROM categorias c WHERE c.id = a.id_categoria) as categoria"
                    . ", (SELECT u.telefone FROM usuarios u WHERE u.id = a.id_usuario) as telefone FROM anuncios a WHERE a.id = ?");
            $sql->execute(array($id));
            if($sql->rowCount()>0){
                $array = $sql->fetch();
                $array['fotos'] = array();
                $sql = $this->db->prepare("SELECT id,url FROM anuncios_imagens WHERE id_anuncio = :id");
                $sql->bindValue(":id",$id);
                $sql->execute();
                if($sql->rowCount()>0){
